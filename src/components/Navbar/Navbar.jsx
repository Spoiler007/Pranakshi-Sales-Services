            ...
            {navbarOpen ?
                ...
                >
                    ...
                    <div className="flex flex-col items-center z-20 justify-center gap-10 text-2xl font-semibold  pt-8">
                        {
                            menus.map((menu) => {
                                return <a key={menu.name} href={menu.link} className="border-none font-outfit rounded-lg w-full py-2 bg-black/50 hover:text-yellowlight  text-white text-center mx-auto">{menu.name}</a>
                            })
                        }
                        <div className="bg-white text-greenlight rounded-xl px-5 py-2 font-semibold  border-2 border-yellowlight hover:border-2 flex gap-2 items-center"> <PiPhoneCall className="text-3xl text-greendark " /> 24/7 Support</div>
                    </div>
                </div>
                :
                <div className={` ${!showNav ? "-translate-y-20" : "translate-y-0"} transition-all duration-500 fixed border-none shadow-gray-500 z-50 w-full bg-gradient-to-b from-greendark to-greenlight shadow-2xl  flex items-center justify-between py-3  px-16`} key={"close"}>
                    <div className="logo flex items-center justify-center font-semibold text-lg font-outfit"><img src="/Images/logo.png" alt="logo" height={90} width={130} className="mr-2 rounded-xl" /> <span className="md:inline text-yellowlight mt-10 sm:flex hidden text-2xl">Sales & Services</span></div>
